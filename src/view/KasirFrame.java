...
public class KasirFrame extends JFrame {
    ...
    private void initializeComponents() {
        // Null check for authController
        String userName = (authController != null && authController.getCurrentUser() != null) 
    ...
    private void setupLayout() {
        ...
        JLabel userLabel = new JLabel("Operator: " + userName);
    ...
    private void setupEventHandlers() {
        ...
        // Double click on menu table to add to order
    ...
    private void loadMenuData() {
        try {
            ...
            // Add some dummy data if controller fails
            addDummyMenuData();
        }
    }
    ...
    private void addDummyMenuData() {
        Object[][] dummyData = {
            {1, "Nasi Goreng", "Makanan", "Rp25,000", "Nasi goreng special"},
            {2, "Ayam Bakar", "Makanan", "Rp30,000", "Ayam bakar bumbu kecap"},
            {3, "Es Teh", "Minuman", "Rp5,000", "Es teh manis"},
            {4, "Jus Jeruk", "Minuman", "Rp8,000", "Jus jeruk segar"},
            {5, "Soto Ayam", "Makanan", "Rp20,000", "Soto ayam kuning"}
        };
    ...
    private void loadCustomerData() {
        try {
            ...
            // Add dummy customer if controller fails
            addDummyCustomer();
        }
    }
    ...
    private void addDummyCustomer() {
        ...
        dummyCustomer.setFullName("Customer Umum");
        customerComboBox.addItem(dummyCustomer);
    }
    ...
    private void addToOrder() {
        ...
        try {
            ...
            BigDecimal price = parseRupiah(priceText);
    ...
    private void updateOrderTable() {
        ...
        totalLabel.setText("Total: " + formatRupiah(currentTotal));
    }
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            try {
                ...
                if (paymentMethod == Order.PaymentMethod.CASH) {
                    handleCashPayment(selectedCustomer, paymentMethod);
                } else {
                    // QRIS payment
                    handleQrisPayment(selectedCustomer, paymentMethod);
                }
            } catch (Exception e) {
                JOptionPane.showMessageDialog(this, 
    ...
    private void handleCashPayment(User customer, Order.PaymentMethod paymentMethod) {
        ...
        if (cashInput != null && !cashInput.trim().isEmpty()) {
            try {
                BigDecimal cashReceived = new BigDecimal(cashInput.replace(".", "").replace(",", ""));
                ...
                // Process the order
                int orderId = processOrderWithController(customer, paymentMethod);
    ...
    private void handleQrisPayment(User customer, Order.PaymentMethod paymentMethod) {
    ...
    private int processOrderWithController(User customer, Order.PaymentMethod paymentMethod) {
        try {
            int cashierId = (authController != null && authController.getCurrentUser() != null) 
                ? authController.getCurrentUser().getId() 
                : 1; // Default cashier ID
                
            return kasirController.processOrder(
                customer.getId(),
                cashierId,
                currentOrderItems,
                paymentMethod
            ...
            // If controller fails, return dummy order ID
    ...
    private BigDecimal parseRupiah(String rupiahText) {
        if (rupiahText == null || rupiahText.isEmpty()) return BigDecimal.ZERO;
        
        // Remove "Rp" and any formatting
        String cleanText = rupiahText.replace("Rp", "").replace(",", "").replace(".", "").trim();
